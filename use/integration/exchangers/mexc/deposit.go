...
func (c *client) DepositNetwork(ctx context.Context, symbol string) (networks []string) {
	...
	query := fmt.Sprintf("%s?%s&signature=%s", "/api/v3/capital/deposit/address", queryToSign, usign.SignRequest(queryToSign, c.cfg.SecretKey))
	...
	var responseBody []response.DepositAddress
	...
	networks = make([]string, 0, len(responseBody))
	for _, a := range responseBody {
